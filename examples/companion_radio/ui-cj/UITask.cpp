...
// main.cpp global; the canned-message file lives on its filesystem.
...
/* ================================================================= ui-cj ==== */
/* Navigation and input for the contact list, message log, and canned sender.

   These live here rather than in CJScreens.h because they call back into UITask,
   which is only a forward declaration at that point. */
...
void UITask::gotoSendScreen() {
  ContactInfo c;
  if (((CJContactsScreen *) cj_contacts)->getSelected(c)) {
    ((CJSendScreen *) cj_send)->setRecipient(c);
    setCurrScreen(cj_send);
  } else {
    showAlert("No contacts", 1000);
    gotoHomeScreen();
  }
}
